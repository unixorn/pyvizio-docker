...
CONTAINER_NAME = 'unixorn/pyvisio'
...
# Tasks
desc 'Clone pyvisio source'
task :clone_pyvisio do
  sh %{ git clone git@github.com:unixorn/pyvizio-docker.git }
end
...
task :debug do
  sh %{ docker buildx build --platform linux/amd64 --load -t #{CONTAINER_NAME}-debug . }
end
